...
//    SAMPLE_SVP_NNIE_Encode
    ...
    finish = clock();
    fprintf(stderr, "load input_bin %d ms\n", (finish - start)/1000);
/*    
...
*/
    ...
    ncnn::ParamDict pd;
    ncnn::Mat encoder_img,encoder_image,pm_encoder,encoder_att;
    pd.set(0, 2);
    ...
    finish = clock();
    fprintf(stderr, "ada_poll cost %d ms\n", (finish - start)/1000);
    ...
    delete permute;
    finish = clock();
    fprintf(stderr, "permute cost %d ms\n", (finish - start)/1000);
    ...
    linear->forward(init_input, init_h, ncnn::get_default_option());    printf("1\n");
    ...
    delete linear;
    finish = clock();
    fprintf(stderr, "init_hc cost %d ms\n", (finish - start)/1000);
...
*/
    ...
    while(true){
        ...
        ex.input("encoder_att", encoder_att);
#ifdef DEBUG
        if(step == 3)
        {
        ...
        clock_t fff = clock();
        fprintf(stderr, "next data cost %d ms\n", (fff - sss)/1000);
...
//        ncnn::Mat next_input_h(DECODER_DIM, k), next_input_c(DECODER_DIM, k);
    ...
    finish = clock();
    fprintf(stderr, "decoder loop cost %d ms\n", (finish - start)/1000);
