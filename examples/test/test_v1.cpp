...
*/
    ...
    //load WORDMAP.json
#ifdef ARM_HISI
    FILE *wordmp_fp = fopen("WORDMAP.json", "r");
#else
    ...
    while(true){
        ...
        ex.extract("attention_weighted_encoding", attention_weighted_encoding);
        ...
        print_data(awt_out, 0, 20, "awt_out");
        print_data(embeddings, 0, 20, "embeddings");
        print_data(lstm_out_h, 1, 20, "lstm_out_h");
        print_data(lstm_out_c, 1, 20, "lstm_out_c");
        }
#endif
...
//        ncnn::Mat next_input_h(DECODER_DIM, k), next_input_c(DECODER_DIM, k);
    ...
    {
        printf("%d ", complete_seqs[final_seq].data()[i]);
    }
    printf("\n");
#ifdef DEBUG
    }
